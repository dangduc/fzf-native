...
The toolchain directive and build scripts require Go =1.27.1=, whose standard
library embeds Unicode =17.0.0= tables.  The native peer vendors utf8proc
=2.10.0= with Unicode =16.0.0= tables.
...
The checks compare Unicode results and full result matrices for all three
scoring schemes.  An exhaustive lowercase audit checks all 1,488 mappings in
the pinned Go tables in both pattern/candidate orientations through all seven
raw algorithms.  Its 20,832 requests accept exactly 28 version differences in
each of the 14 orientation/algorithm cells: U+A7CE, U+A7D2, U+A7D4, and
U+16EA0--U+16EB8.  Any other difference, or any change to those counts, fails
the audit.
