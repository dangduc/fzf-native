...
(defun fzf-native-upstream--env-integer (name default)
  "Read non-negative integer NAME from the environment, or use DEFAULT."
...
(defun fzf-native-upstream--seed (seed)
  "Initialize the deterministic PRNG with SEED."
...
(defun fzf-native-upstream--term ()
  "Generate one fzf extended-search term."
  (concat (aref ["" "" "'" "^" "!"]
...
(defun fzf-native-upstream--keys (strings)
  "Return a sorted, property-free multiset representation of STRINGS."
...
(defun fzf-native-upstream--fzf (fzf collection query case-mode fuzzy)
  "Return matches from FZF for COLLECTION and QUERY."
  (let ((args (append
               ...
               (pcase case-mode
                 ...
                 (_ '("--smart-case")))
               (unless fuzzy '("--exact"))))
        (input (mapconcat #'identity collection "\0"))
        (output (generate-new-buffer " *fzf-native-upstream*")))
    (unwind-protect
        (with-temp-buffer
          ;; A final delimiter is required to preserve a final empty candidate.
          (insert input "\0")
          (let ((coding-system-for-write 'utf-8-unix)
                (coding-system-for-read 'utf-8-unix)
                (status (apply #'call-process-region
                               (point-min) (point-max) fzf nil output nil args)))
            (unless (memq status '(0 1))
              (error "%s exited with status %S" fzf status)))
          (with-current-buffer output
            ;; --print0 terminates every match.  Drop only that final sentinel;
            ;; interior empty strings represent real empty candidates.
            (butlast (split-string (buffer-string) "\0" nil))))
      (kill-buffer output))))

(ert-deftest fzf-native-fuzz-upstream-match-set ()
  "Compare randomized match multisets with a pinned upstream fzf CLI."
  (let* ((fzf (or (getenv "FZF_REFERENCE") (executable-find "fzf")))
         ...
         (fixed '("" "a" "A" "alpha" " foo" "foo " "foo.bar" "CAFÉ"
                  "café" "é" "Kelvin" "kelvin" "你好世界" "你是" "λambda"
                  "Жук" "א" "🚀 launch")))
    (skip-unless fzf)
    ...
    (dotimes (iteration cases)
      (let* ((fzf-native-case-mode
              (aref [smart ignore respect] (fzf-native-upstream--random 3)))
             (fzf-native-fuzzy (not (zerop (fzf-native-upstream--random 2))))
             (collection
              (append fixed
             ...
             (native (fzf-native-upstream--keys
                      (fzf-native-score-all
                       (mapcar #'copy-sequence collection) query)))
             (upstream (fzf-native-upstream--keys
