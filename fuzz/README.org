...
Raw algorithm normalization uses the pinned fzf Latin-fold table.
The Emacs API does not expose normalization yet.
Scoring, ranking, backward search, boundary syntax, and that API gap are parity debt.
