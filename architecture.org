...
Every platform exposes module ABI version 2.
Windows keeps batch-only support, but the scoring scheme changes its batch results.

=fzf-native.el= checks =fzf-native-abi-version= after a module loads.
The loader rejects a missing or incompatible module ABI.
It repeats this check when =fzf-native-loaded= is already non-nil.
...
| =(fzf-native-abi-version)= | The module ABI version on every platform. |
...
| ='foo'= | Exact match with word boundaries. |
| =^foo= | Prefix match. |
| =foo$= | Suffix match. |
| =^foo$= | Equal match. |
| =!foo= | Inverse match. |
...
=fzf-native-normalize= uses fzf's Latin normalization by default.
Set it to nil for the behavior of fzf's =--literal= option.

Normalization depends on the query term.
An ASCII term can match its normalized Latin forms.
An accented term stays literal when normalization would change its lowercase form.

=fzf-native-search-direction= selects =auto=, =forward=, or =backward=.
Auto uses backward search for the path scheme and forward search otherwise.
...
The default scheme uses this total order:

1. Higher score.
2. Shorter trimmed character length.
3. Earlier producer index.

For scoring, the path scheme treats slash as a delimiter on every host and
backslash as an additional delimiter on Windows.

The path scheme adds pathname distance before character length.
Pathname distance starts after the last slash or backslash before the match.

The history scheme uses score and producer index only.
Inverse-only patterns preserve producer order for every scheme.

Length counts decoded characters after Unicode-space trimming.
It saturates at 65,535, as fzf does.

Batch and session paths use the same total order.
The order stays deterministic across worker completion order.
...
When complete membership and a covering top K exist, the retry scores only the
appended delta.  It merges the new scores into the cached top K and extends the
membership with an immutable delta node.

With complete membership but no covering top K, the retry scans prior members
and the appended delta.  Without complete membership, it uses stable-batch
evidence or scans the selected pool boundary.
...
The worker sets the request score scheme before it uses the slab.
...
- literal query
- case mode, fuzzy mode, normalization mode, resolved direction, and score scheme
- filter-only mode
...
Complete membership uses a reference-counted =SharedIdx= chain.  Producer growth
shares the immutable prior membership and stores only new matching indexes.
At 32 nodes, the chain can retain its oldest 16 nodes and compact the newer
suffix into one immutable node.  After two suffix-only compactions, the next
depth-limit event forces a full flatten and resets the counter.  This bound
prevents a fixed retained prefix from causing unbounded repeated suffix copies.
The chain also falls back to a full flatten when the suffix representation
exceeds the membership byte budget.
...
The key contains query bytes and all matching options.
It also contains the stable batch ID.
...
| =:normalize= | The retained result normalization mode. |
| =:search-direction= | The retained result resolved direction. |
| =:score-scheme= | The retained result score scheme. |
...
It receives an empty vector when there are no positive highlight positions.
That vector does not encode membership: inverse-only matches and misses both produce it.
...
| Variable | Read point |
|----------+------------|
| =fzf-native-score-scheme= | Each match, score, or highlight call. |
| =fzf-native-case-mode= | Every scoring request. |
| =fzf-native-fuzzy= | Every scoring request. |
| =fzf-native-normalize= | Every scoring request. |
| =fzf-native-search-direction= | Every scoring request. |
...
An fzfa bridge must bind normalization, direction, and scheme together.
Its request key must contain their effective values.
