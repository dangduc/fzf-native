...
- *Simple positive byte-prefix* — Q' is one positive term and is a byte-prefix
  ...
  terms.  Other operator-bearing sources must use parsed-term proof because an
  inverse extension can broaden a match set.  Empty, doubled, inverse, anchored,
  and OR-bearing quoted forms do not use this fast path.
