...
Driven by =fzf-native-filter-only-min-pool=.  When the snapshot pool
reaches at least that size, the scoring thread swaps its per-candidate
work from =fzf_get_score= (full fuzzy-DP, ~500 ns per candidate) to
=fzf_has_match= (boolean match-only check from =fzf-additions.c=,
~100 ns per candidate).  The candidate cap at the display layer
(=fzf-async-max-candidates=, default 10 000) is then re-scored with
=fzf_get_score= and counting-sorted so the user still sees results
ordered by score — but only that 10 K window pays the DP cost, not
the whole pool.
...
- =fzf-native-filter-only-min-pool= (nil/0 disables; 1 always-on for
  testing; default 10 000 000).
- Threshold checked per scoring run against the snapshot pool size.  A
  streaming session crosses modes naturally as the reader grows the
  pool past the threshold.
