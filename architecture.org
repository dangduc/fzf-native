...
- Async: each persistent worker owns one slab for the session lifetime.
...
#+begin_src
  loop:
    ...
    publish one job epoch to the persistent worker pool
    wait for all pool workers at the completion condition
...
The batch loop also checks abort every 64 K items during snapshot
construction.  Thus, a filter change can stop a 25 M-item copy early.

** Persistent worker pool

=fzf-native-async-start= creates the worker pool before it starts the reader
and scorer.  The pool uses at most 64 online CPUs.

Each worker waits for a new job epoch on =job_cond=.  It owns one matcher slab
and reuses that slab for all request rounds.

The scorer publishes a shared job and broadcasts =job_cond=.  Workers claim
batches through the atomic =remaining= counter.

After a worker completes a full batch, it updates the atomic progress counter.
The increment is the original candidate count of that batch.

The last worker signals =done_cond=.  The scorer then compacts, ranks, caches,
and publishes the completed result.
...
thread.  Configure it with =fzf-native-async-cache-size=.  Higher-level
...
single-owner rule prevents two threads from waiting for the same PID.  A
snapshot reports =:producer-state=, =:producer-exit-status=, and
=:producer-error=.
...
| Worker slabs            | Persistent workers              | Created at session start and freed at session stop |
...
1. Set =score_abort=, =score_req_stop=, and =stop=.  Then signal the scorer.
2. Stop the producer so that the reader gets EOF.
3. Join the scorer after the active worker epoch stops.
4. Stop and join all persistent workers.
5. Join the reader before destruction of =score_req_mu= and =score_req_cond=.
6. Free the cache, candidate arena, result data, locks, and session.
...
|---------------------------------------------------------------------+---------------------------------------|
| Query submission creates no process or worker thread.                      | The session creates the producer and worker pool at start. |
