...
The =:score-scheme= field identifies the scheme for the retained list.
The =:normalize= and =:search-direction= fields identify its matching policy.
...
On every platform, =fzf-native.el= checks =fzf-native-abi-version= during module loading.
A stale module fails with a rebuild and restart instruction.
This release needs ABI 2 artifacts for every supported platform.
...
|----------+---------+------------+---------|
| =fzf-native-score-scheme= | =default= | Each match, score, or highlight call | Select =default=, =path=, or =history= bonuses. |
...
| =fzf-native-normalize= | t | Every scoring request | Use fzf Latin normalization.  Nil matches fzf =--literal=. |
| =fzf-native-search-direction= | =auto= | Every scoring request | Use backward search for =path= and forward search for other schemes. |
...
The =default= scheme gives the largest bonus to a match after whitespace.
The =path= scheme gives the largest bonus to a match after a slash on every
host and after a backslash on Windows.
The =history= scheme gives equal bonuses to whitespace and delimiters.

The default scheme ranks by score, trimmed character length, and producer order.
The path scheme also ranks by the distance from the last slash or backslash.
The history scheme ranks by score and producer order.

Normalization is directional, as it is in fzf.
For example, =e= can match =é=, but =é= remains a literal query.

The scheme, normalization setting, and resolved direction are request identities.
They are also part of both cache identities.
Thus, one query can keep separate cached results for each matching policy.

An fzfa integration must bind all three options at each native call site.
It must also include their effective values in its asynchronous request key.
...
The project license is in [[file:LICENSE][LICENSE]] (GNU GPL). The MIT license for
the fzf code derived from fzf and telescope-fzf-native.nvim is in
[[file:LICENSE-MIT][LICENSE-MIT]]. Vendored utf8proc and Unicode data notices are in
[[file:utf8proc-2.10.0/LICENSE.md][utf8proc-2.10.0/LICENSE.md]] and the corresponding source files.
