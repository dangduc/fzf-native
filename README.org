...
~running~, ~complete~, ~failed~, ~cancelled~, ~idle~, or ~unknown~.  The plist keeps the
...
The ~:snapshot-generation~ field increments for each published result.
While a request runs, ~:progress-completed~ reports the number of candidates
in fully scanned batches.  ~:progress-total~ reports the size of that scan.
...
Each asynchronous session owns one persistent scoring pool.  Query updates do
not start a new process or create new scoring threads.
...
| ~fzf-native-async-batch-cache-bytes~ | 64 MiB    | ~fzf-native-async-start~ (once at session start) | Byte budget for safe membership data from complete stable batches.  Set to 0 to disable. |
...
The fuzz suite has four complementary local layers:

- ~make fuzz-replay~ replays the checked-in UTF-8, query-parser, inverse-term,
  small-slab, and interactive-session regression corpora under
  AddressSanitizer and UndefinedBehaviorSanitizer.
- ~make fuzz-matcher~ runs the structured matcher harness with libFuzzer.  New
  coverage lives in ~build/fuzz-corpus~ and minimized failures in
  ~build/fuzz-artifacts~.
  In addition to matcher-level invariants, the harness rewrites extended-search
  queries and checks whitespace equivalence, AND/OR reordering, duplicate-term
  idempotence, and the duality between a leading quote and the global fuzzy
  mode.  Score equality is required where term evaluation order is irrelevant;
  otherwise the oracle compares match membership.
- ~make fuzz-session~ drives the real native interactive session engine from a
  compact operation trace.  It covers candidate growth, request replacement,
  cancellation, worker scoring, exact and batch caches, publication, polling,
  and active teardown.  Every quiescent snapshot is compared with an
  independently sorted batch result at the same pool boundary.  Its corpus and
  artifacts live in ~build/fuzz-session-corpus~ and
  ~build/fuzz-artifacts/session~.
- ~make fuzz-elisp~ checks scalar/batch/filter/highlight equivalence and
  exercises async query refinement, broadening, term reordering, garbage
  collection, and teardown.  Runs are deterministic; set
  ~FZF_NATIVE_FUZZ_SEED~, ~FZF_NATIVE_FUZZ_CASES~, and
  ~FZF_NATIVE_FUZZ_ASYNC_SESSIONS~ to reproduce or deepen them.  Set
  ~FZF_NATIVE_FUZZ_TRACE_ITERATION~ to print one exact generated ABI case
  before it executes (useful when reducing a signal-level failure).

~make fuzz~ runs both native libFuzzer targets.  Set ~FUZZ_SECONDS~ to control
each target's run length.  ~make fuzz-continuous~ runs and coverage-merges both
targets in repeated 30-minute epochs; set ~FUZZ_EPOCH_SECONDS~ to change the
per-target epoch length.

When an upstream ~fzf~ executable is available, ~make fuzz-upstream~ compares
randomized match multisets with the reference CLI.  CI pins both its release
and archive checksum.  Local runs can select the same reference explicitly:

#+begin_src bash
FZF_REFERENCE=/path/to/fzf FZF_REFERENCE_VERSION=0.74.3 make fuzz-upstream
#+end_src

The separate ~make ctest-tsan~ lane covers the async reader, result cache,
deferred teardown, and interactive trace corpus under ThreadSanitizer.  Every
fuzz failure prints its seed or writes a replayable artifact; promote minimized
permanent regressions into ~fuzz/corpus~, ~fuzz/session-corpus~, or a focused
unit test.
