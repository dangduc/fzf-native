...
The fuzz suite has three complementary layers:

- ~make fuzz-replay~ replays the checked-in UTF-8, query-parser, inverse-term,
  and small-slab regression corpus under AddressSanitizer and
  UndefinedBehaviorSanitizer.
- ~make fuzz~ runs the same structured matcher harness with libFuzzer.  Set
  ~FUZZ_SECONDS~ to control the run length.  New coverage lives in
  ~build/fuzz-corpus~ and minimized failures in ~build/fuzz-artifacts~.
  ...
  ~make fuzz-continuous~ runs repeated 30-minute epochs.  Each epoch keeps the
  corpus and resets the sanitizer state.  A failure stops the loop.  Set
  ~FUZZ_EPOCH_SECONDS~ to change the epoch length.
...
The separate ~make ctest-tsan~ lane covers the async reader, result cache, and
deferred teardown under ThreadSanitizer.  Every fuzz failure prints its seed
or writes a replayable artifact; promote minimized permanent regressions into
~fuzz/corpus~ or a focused unit test.
